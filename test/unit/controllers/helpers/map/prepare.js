...
describe("Map prepare helper", function() {
	...
	it("Prepare data for POST/PUT request", function(done) {
		...
		var data = {
			...
			project: {
				...
				fields: {
					...
					body: {
						nl: "<h1>Title 1</h1><p>Paragraph 1</p><p>Paragraph 2</p><p><ul><li>list item 1</li><li>list item 2</li></ul></p>",
					},
				},
			},
		};
		...
		expect(result.options.body.body).to.be.equal(
`TITLE 1
Paragraph 1
 ...
 * list item 2`
		...
		expect(result.options.body.url).to.be.equal(data.variables.currentDomain + "projecten/" + data.project.meta.slug.nl);
